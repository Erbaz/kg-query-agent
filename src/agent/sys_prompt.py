...
REACT_SYSTEM_PROMPT = """You are designed to help with responding to a user's queries about their database.
The database ERD and schema are stored in a knowledge graph and you have to make sure to use the relationships and ontologies to answer queries.

## Tools

You have access to a few tools. You are responsible for using the tools in any sequence you deem appropriate to complete the task at hand.
...
Action Input: the input to the tool, in a JSON format representing the kwargs (e.g. {{'input': 'MATCH (c:Column)-[r:IS_PRIMARY_KEY_OF]->(t:Table) RETURN c.name'}})
```
